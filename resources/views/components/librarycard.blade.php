    ...
    <div class="overflow-hidden text-left rounded-md shadow-lg md:w-40 w-28 md:h-fit card h-fit" style="background-color: rgba(235, 235, 235, 0.7)">
      ...
      <figure class="py-2 text-center">
        <blockquote class="w-full">
          <p class="w-40 overflow-hidden text-md md:text-xl text-ellipsis whitespace-nowrap">{{ $book->title }}</p>
        </blockquote>
      </figure>
      <div class="px-2 pb-2 text-center md:px-6">
        <span class="block px-3 py-1 text-xs font-semibold text-center text-gray-700 bg-gray-200 rounded-xl md:text-md align-self-center">{{ $book->genre->name }}</span>
      </div>
    </div>
</button>
    ...
    <div class="relative w-full max-w-md max-h-full rounded-lg bg-fuchsia-900">
        ...
        <div class="flex flex-col p-6 text-center rounded-2xl bg-fuchsia-900">
            ...
            <div class="flex items-center mx-2">
                ...
                <a data-modal-hide="{{ $book->id }}" href="{{ route('book.description', ['bookID' => $book->id ]) }}" class="font-sans text-lg text-white hover:text gray-100">Info</a>
            </div>
